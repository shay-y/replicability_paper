...
(2) Run R and install the following packages (needed only once) with the command:
    ...
    install.packages('rJava','XLConnect','xtable','nlme','multcomp','knitr','dplyr','lubridate')
...
(3) Install mikTex by running the following:
    ...
    or manually from: http://miktex.org/download
    
Reproducing the Analyses and the Simulation:
--------------------------------------------

(1) Download "Replicability_paper.zip" from:
    http://replicability.tau.ac.il/images/Replicability_Paper.zip

(2) Extract the files to a folder named "Replicability_paper". This folder is the working directory for the knitr R processes.

(3) Open the Replicability_Paper.Rproj to begin a new RStudio project in the folder.
...
(5) Click "compile PDF" on the top of the open script window to run the script and compile a pdf with a report resulted from the analysis.
    The pdf outputs to the same directory, along with TeX file and log file of the Latex compiling process.

Troubleshooting:
----------------
    
(*) Make sure that RStudio uses knitr for waeving (should be the default). Follow the steps:
...
(*) The "wahlsten datasets" document compiling may take between 10 minutes to 1 hour, depending on the PC.
